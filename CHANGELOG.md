...
- **AuthScreen error alert design token** — replaced hardcoded `bg-red-50 border-red-200` with opacity-based `bg-red-500/10 border-red-500/20` on the error alert for design system consistency
- **Focus rings (round 5)** — comprehensive keyboard focus pass across 10 components that had zero `focus-visible:ring` styles: VideoWall (mute, nav arrows), FeedItem (12 buttons — avatar, username, emoji picker, emoji options, reaction counts, reply, show replies, send reply, like, comments, send comment), HomeTab (profile avatar, new event, scroll left/right, refresh), TribeDiscovery (close, join), MangoChat (close, quick prompts, view event, send), ProfileTab (11 buttons — menu items, log out, back, sound/experimental toggles, group chats, fame card, become organiser, go pro), AuthScreen (testimonial dots, prev/next, submit, toggle register/login), LevelUpModal (close, continue), LevelUpScreen (close, keep going)
- **Scroll containment** — added `overscroll-contain no-scrollbar` to 11 scrollable areas in bottom sheets and modals (MyBookingsSheet, SavedEventsSheet, HelpSheet, TribeSheet, GroupChatsSheet x2, MangoChat, OnboardingFlow, TribeDiscovery, BugReportModal, AppModals, EventDetailSheet) to prevent iOS scroll bounce from leaking to the parent and hide scrollbars
- **Decorative aria-hidden (round 4)** — added `aria-hidden="true"` to VideoWall gradient overlay and AvatarCropModal crop guide border
...
- **Focus ring consistency (final pass)** — LevelUpScreen close and CTA buttons now use `focus-visible:ring-primary/30` instead of `focus-visible:ring-white/30`. EventDetailSheet chat avatar button fixed from `focus:ring` to `focus-visible:ring`. EventDetailSheet send message and join buttons now have `focus-visible:ring` for keyboard navigation. VideoWall mute button and OrganiserEditProfileSheet cover photo remove button also standardised to `primary/30`
- **Fake interactivity removal (post-rebase)** — removed `cursor-pointer` from VideoWall CuratedIntroCard (no click handler) and AuthScreen "Community Pact" text (no link). Connections Card now opens bookings on tap with proper keyboard support (`role="button"`, `tabIndex`, Enter/Space handler)
